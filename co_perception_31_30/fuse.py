...
if __name__ == '__main__':
    ...
    master_pcfile = open('ego1_lidar_measurement_2089.txt', 'r')
    assist_pcfile = open('ego2_lidar_measurement_2090.txt', 'r')
    fuse_pcfile_name = 'ego12_2091.txt'

    ego1_loc = np.asarray([-84.76717376708984,37.042301177978516,0.23253661394119263])
    ego1_rot = np.asarray([-0.0019225535215809941,1.3693435192108154,89.78823852539062])
    ego2_loc = np.asarray([-88.16752624511719,36.95361328125,0.23052169382572174])
    ego2_rot = np.asarray([-0.0019225978758186102,1.3580259084701538,89.78831481933594])

    print(ego2_loc-ego1_loc, ego2_rot-ego1_rot)

    master_point_cloud = point_cloud(master_pcfile)
    assist_point_cloud = point_cloud(assist_pcfile)

    assist_point_cloud.rotation(ego2_rot-ego1_rot)
    assist_point_cloud.translation(ego2_loc-ego1_loc)
